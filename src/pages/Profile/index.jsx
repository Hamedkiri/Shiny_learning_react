...
function Profile() {
  const [freelanceData, updateFreelancesData] = useState([])
  const { id } = useParams()
  useEffect(() => {
    fetch(`http://localhost:8000/freelance?id=${id}`).then((response) =>
      response
        .json()
        .then((jsonResponse) =>
          updateFreelancesData(jsonResponse?.freelanceData)
        )
    )
  }, [])
  const { picture, name, location, tjm, job, skills, available } = freelanceData
